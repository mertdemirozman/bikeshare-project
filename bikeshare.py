...
def get_filters():
    ...
    """
    print('Hi there! Lets look at the United States bikeshare data! ')
    print()
    # TO DO: get user input for city (chicago, new york city, washington). HINT: Use a while loop to handle invalid inputs
    while 1:
        print('Which city do you want to analyse ? You can choose between Chicago, New York and Washington')
        ...
        if city not in CITY_DATA:
            print('There is something not true about your typing. Please check again')
            continue
        city = CITY_DATA[city]
        break
    ...
    while 1:
        anyfilter = input('Do you want to filter the data by month and/or week? Yes/No ').lower()
        print()
        if anyfilter=='yes' or anyfilter=='y':
            anyfilter=True
        elif anyfilter=='no' or anyfilter=='n' :
            anyfilter=False
        else:
    ...
    while 1:
        if anyfilter:
            filter=input('Do you want to filter by month, day or both?').lower()
            print()
            if filter=='month':
                ...
                if month not in MONTH_DATA:
                    print('There is no valid input. Can you try it again?')
                    continue
                month = MONTH_DATA[month]
                day='all'
            elif filter=='day':
                print('Which day do you want to analyse? ')
                ...
                if day not in WEEK_DATA:
                    print('Invalid typing. Please try it again.')
                    continue
                day = WEEK_DATA[day]
                month='all'
            elif filter=='both':
